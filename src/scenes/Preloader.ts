...
export class Preloader extends Phaser.Scene {
  ...
  private buildTileset(): void {
    ...
    const dots = (offX: number, pairs: number[][], col: string) => {
      ctx.fillStyle = col;
    ...
    ctx.fillRect(S+2, 5, 2, 1); ctx.fillRect(S+9, 11, 2, 1);
    ...
    ctx.fillRect(S*2+2, 2, 2, 1); ctx.fillRect(S*2+9, 9, 2, 1);
    ...
    ctx.fillRect(S*3+5, 2, 1, 1); ctx.fillRect(S*3+12, 13, 1, 1);
    ...
    ctx.fillStyle = '#ff4466'; ctx.fillRect(S*4+2, 7, 1, 1);
    // Yellow flower
    ctx.fillStyle = '#ccaa00'; ctx.fillRect(S*4+10, 3, 2, 2);
    ctx.fillStyle = '#ffdd00'; ctx.fillRect(S*4+10, 3, 1, 1);
    // Stems
  ...
  private buildPlayerSprite(): void {
    ...
    interface FrameOpts {
      bodyCol : string;
      legOff  : number;   // leg animation offset (0 or 1)
      eyeShift: number;   // eye position horizontal shift
      mouthOpen: boolean;
    }

    const drawChar = (fi: number, o: FrameOpts) => {
      const ox = fi * FW;
  ...
  private buildNPCSprite(): void {
    ...
    const drawNPC = (fi: number, bodyCol: string, hairCol: string, legOff: number) => {
      const ox = fi * FW;
      ctx.fillStyle = '#2244aa'; ctx.fillRect(ox+3, 14+legOff, 2, 2-legOff); ctx.fillRect(ox+7, 14-legOff, 2, 2+legOff);
      ctx.fillStyle = bodyCol;   ctx.fillRect(ox+2, 5, 8, 9);
      ctx.fillStyle = '#fdbcb4'; ctx.fillRect(ox+3, 1, 6, 5);
      ctx.fillStyle = hairCol;   ctx.fillRect(ox+3, 0, 6, 3);
