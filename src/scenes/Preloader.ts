...
export class Preloader extends Phaser.Scene {
  ...
  private buildBattleBackgrounds(): void {
    this.buildBgField();
    this.buildBgForest();
    this.buildBgDungeon();
    this.buildBgVillage();
  ...
  private buildBgField(): void {
    ...
    // Clouds
    const drawCloud = (cx2: number, cy2: number, w2: number) => {
      ctx.fillStyle = '#FFFFFF';
      ctx.globalAlpha = 0.9;
      ctx.beginPath(); ctx.ellipse(cx2, cy2, w2, w2 * 0.45, 0, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.ellipse(cx2 - w2 * 0.4, cy2 + 4, w2 * 0.55, w2 * 0.35, 0, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.ellipse(cx2 + w2 * 0.35, cy2 + 3, w2 * 0.5, w2 * 0.32, 0, 0, Math.PI * 2); ctx.fill();
      ctx.globalAlpha = 1;
    };
    drawCloud(55, 22, 28);
    drawCloud(150, 15, 22);
  ...
  private buildBgForest(): void {
    ...
    ctx.beginPath(); ctx.arc(35, 28, 12, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#1A2838';
    ctx.beginPath(); ctx.arc(39, 25, 10, 0, Math.PI * 2); ctx.fill();
    ...
    ctx.beginPath(); ctx.arc(85, 138, 6, Math.PI, 0); ctx.fill();
    ctx.fillStyle = '#882222';
    ctx.fillRect(84, 138, 2, 8);
    ctx.fillStyle = '#FF8888';
    ctx.fillRect(83, 138, 1, 2); ctx.fillRect(88, 135, 1, 2);
    ...
    ctx.lineWidth = 3;
    ctx.beginPath(); ctx.moveTo(20, 80); ctx.lineTo(50, 65); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(236, 75); ctx.lineTo(210, 60); ctx.stroke();
  ...
  private buildBgDungeon(): void {
    ...
    ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(50, 5); ctx.lineTo(60, 20); ctx.lineTo(70, 15); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(180, 8); ctx.lineTo(170, 22); ctx.stroke();
    ...
    const drawBricks = (wx: number, wy: number, ww: number, wh: number) => {
      ctx.strokeStyle = '#151010';
      ctx.lineWidth = 1;
  ...
  private buildBgVillage(): void {
    ...
    const drawBuilding = (bx: number, by: number, bw: number, bh: number, roofH: number, wallCol: string, roofCol: string) => {
      // Roof
    ...
    ctx.beginPath(); ctx.arc(128, 62, 5, 0, Math.PI * 2); ctx.fill();
  ...
  private buildNPCSprites(): void {
    ...
    const mkHead = (hairC: string, eyeC: string, label: string) => {
      void label;
      return [
  ...
  private buildEnemySprite(): void {
    ...
    // ── Slime (frames 0-2) ──────────────────────────────────────────────
    const slimeFrame = (fi: number, bounce: number) => {
      const ox = fi * EW;
      const bodyH = 12 + bounce;
      const bodyY = 10 - bounce;
      ctx.fillStyle = '#001100'; ctx.globalAlpha = 0.3;
      ctx.beginPath(); ctx.ellipse(ox + 10, 23, 7, 2, 0, 0, Math.PI * 2); ctx.fill();
      ctx.globalAlpha = 1;
      ...
      // Eyes
      ctx.fillStyle = pal['k']!;
      ctx.fillRect(ox + 4, bodyY + 4, 3, 2); ctx.fillRect(ox + 11, bodyY + 4, 3, 2);
      ctx.fillStyle = pal['w']!;
      ctx.fillRect(ox + 4, bodyY + 4, 1, 1); ctx.fillRect(ox + 11, bodyY + 4, 1, 1);
      // Smile
      ctx.fillStyle = pal['k']!;
      ctx.fillRect(ox + 6, bodyY + 8, 1, 1); ctx.fillRect(ox + 7, bodyY + 9, 4, 1); ctx.fillRect(ox + 11, bodyY + 8, 1, 1);
      // Drip
      if (bounce > 0) {
        ctx.fillStyle = pal['G']!;
    ...
    slimeFrame(0, 0); slimeFrame(1, 2); slimeFrame(2, 3);

    // ── Bat (frames 3-5) ────────────────────────────────────────────────
    const batFrame = (fi: number, phase: number) => {
      const ox = fi * EW;
      ...
      // Eyes glow
      ctx.fillStyle = pal['r']!; ctx.fillRect(ox + 7, 9, 2, 1); ctx.fillRect(ox + 11, 9, 2, 1);
      ctx.fillStyle = pal['e']!; ctx.fillRect(ox + 7, 9, 1, 1); ctx.fillRect(ox + 11, 9, 1, 1);
      ctx.fillStyle = '#FFFFFF'; ctx.fillRect(ox + 7, 14, 1, 2); ctx.fillRect(ox + 10, 14, 1, 2);
      ctx.fillStyle = '#FFFFFF'; ctx.fillRect(ox + 8, 14, 1, 1);
    };
    batFrame(3, 0); batFrame(4, 1); batFrame(5, 2);

    // ── Golem (frames 6-8) ──────────────────────────────────────────────
    const golemFrame = (fi: number, pose: number) => {
      const ox = fi * EW;
      paint(ox, 3, [
        '....oOOOOOOOoo......',
        '...oOAAOOOAAOOo.....',
      ...
      // Glowing eyes
      ctx.fillStyle = pal['F']!; ctx.fillRect(ox + 4, 7, 3, 2); ctx.fillRect(ox + 12, 7, 3, 2);
      ctx.fillStyle = pal['f']!; ctx.fillRect(ox + 4, 7, 1, 1); ctx.fillRect(ox + 12, 7, 1, 1);
      // Cracks
      ctx.fillStyle = pal['C']!;
      ctx.fillRect(ox + 7, 11, 5, 1); ctx.fillRect(ox + 8, 12, 1, 2); ctx.fillRect(ox + 10, 12, 2, 1);
      // Rocks on shoulders
      ctx.fillStyle = pal['o']!;
      ctx.fillRect(ox + 1, 6, 2, 2); ctx.fillRect(ox + 17, 6, 2, 2);
    };
    golemFrame(6, 0); golemFrame(7, 1); golemFrame(8, 2);

    // ── Crystal Wraith (frames 9-11) ────────────────────────────────────
    const wraithFrame = (fi: number, phase: number) => {
      const ox = fi * EW;
      const oy = phase;
      paint(ox, oy + 2, [
        '.....zuuuzz.........',
      ...
      ctx.fillStyle = pal['Q']!; ctx.fillRect(ox + 9, oy + 8, 3, 3);
      ctx.fillStyle = pal['W']!; ctx.fillRect(ox + 10, oy + 8, 1, 1);
      ctx.fillStyle = pal['Y']!; ctx.fillRect(ox + 6, oy + 5, 2, 1); ctx.fillRect(ox + 12, oy + 5, 2, 1);
      // Crystal shards
      if (phase > 0) {
        ctx.fillStyle = pal['Z']!;
      ...
      ctx.fillStyle = glow;
      ctx.beginPath(); ctx.arc(ox + 10, oy + 10, 8, 0, Math.PI * 2); ctx.fill();
    };
    wraithFrame(9, 0); wraithFrame(10, 1); wraithFrame(11, 2);
  ...
  private buildPortraitTextures(): void {
    const defs: Array<[string, string, string, string, string, string]> = [
    ...
    defs.forEach(([key, bg, hairD, hairL, eyeC, clothC]) => {
      ...
      // Eyes
      c.fillStyle = '#1A0C00';
      c.fillRect(9, 12, 4, 2); c.fillRect(19, 12, 4, 2);
      c.fillStyle = eyeC;
      c.fillRect(10, 12, 2, 2); c.fillRect(20, 12, 2, 2);
      c.fillStyle = '#FFFFFF';
      c.fillRect(10, 12, 1, 1); c.fillRect(20, 12, 1, 1);

      // Eyebrows
      c.fillStyle = hairD;
      c.fillRect(9, 10, 5, 1); c.fillRect(18, 10, 5, 1);
      ...
      // Ears
      c.fillStyle = '#F4C090';
      c.fillRect(6, 13, 2, 4); c.fillRect(24, 13, 2, 4);
      ...
      c.fillRect(5, 24, 3, 8); c.fillRect(24, 24, 3, 8);
  ...
  private buildEnvironmentSprites(): void {
    ...
    const palB: Record<string, string | null> = {
      '.': null,
    ...
    const palG: Record<string, string | null> = {
      '.': null,
    ...
    const palS: Record<string, string | null> = { '.': null, Y: '#AACCFF', W: '#FFFFFF', y: '#88AADD', Z: '#6688CC' };
  ...
  private buildUIAssets(): void {
    ...
    ctxC.fillRect(0, 6, 8, 2); ctxC.fillRect(6, 0, 2, 8);
