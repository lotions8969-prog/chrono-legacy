...
export class Preloader extends Phaser.Scene {
  ...
  private buildNPCSprites(): void {
    ...
    const elderF = (fi: number, legV: number) => paint(fi * FW, [
  ...
  private buildEnemySprite(): void {
    ...
    const slime = (fi: number, bounce: number) => {
      ...
      // Shadow
      ctx.fillStyle = '#001100'; ctx.globalAlpha = 0.35;
      ctx.beginPath(); ctx.ellipse(ox + 8, 19, 5, 2, 0, 0, Math.PI * 2); ctx.fill();
      ctx.globalAlpha = 1;
      ...
      // Eyes
      ctx.fillStyle = pal['k']!; ctx.fillRect(ox + 4, bodyY + 3, 2, 2); ctx.fillRect(ox + 9, bodyY + 3, 2, 2);
      ctx.fillStyle = pal['w']!; ctx.fillRect(ox + 4, bodyY + 3, 1, 1); ctx.fillRect(ox + 9, bodyY + 3, 1, 1);
      // Smile
      ctx.fillStyle = pal['k']!;
      ctx.fillRect(ox + 5, bodyY + 6, 1, 1); ctx.fillRect(ox + 6, bodyY + 7, 3, 1); ctx.fillRect(ox + 9, bodyY + 6, 1, 1);
    };
    slime(0, 0); slime(1, 2);
    ...
    const bat = (fi: number, wingsUp: boolean) => {
      ...
      // Eyes
      ctx.fillStyle = pal['r']!; ctx.fillRect(ox + 6, 9, 2, 1); ctx.fillRect(ox + 9, 9, 2, 1);
      ctx.fillStyle = pal['e']!; ctx.fillRect(ox + 6, 9, 1, 1); ctx.fillRect(ox + 9, 9, 1, 1);
      // Fangs
      ctx.fillStyle = '#FFFFFF'; ctx.fillRect(ox + 7, 13, 1, 1); ctx.fillRect(ox + 9, 13, 1, 1);
    };
    bat(2, true); bat(3, false);
    ...
    const golem = (fi: number, armUp: boolean) => {
      ...
      ctx.fillStyle = pal['F']!; ctx.fillRect(ox + 4, 7, 2, 2); ctx.fillRect(ox + 10, 7, 2, 2);
      ctx.fillStyle = pal['f']!; ctx.fillRect(ox + 4, 7, 1, 1); ctx.fillRect(ox + 10, 7, 1, 1);
      ctx.fillStyle = pal['C']!; ctx.fillRect(ox + 5, 11, 6, 1); ctx.fillRect(ox + 6, 12, 1, 1); ctx.fillRect(ox + 8, 12, 1, 1);
    };
    golem(4, false); golem(5, true);

    // ── Crystal Wraith (frames 6-7) ─────────────────────────────────────
    const wraith = (fi: number, phase: number) => {
      const ox = fi * EW;
      ...
      // Glowing core
      ctx.fillStyle = pal['Q']!; ctx.fillRect(ox + 7, oy + 9, 2, 2);
      ctx.fillStyle = pal['W']!; ctx.fillRect(ox + 8, oy + 9, 1, 1);
      // Ethereal eyes
      ctx.fillStyle = pal['Y']!; ctx.fillRect(ox + 5, oy + 6, 2, 1); ctx.fillRect(ox + 9, oy + 6, 2, 1);
      // Trailing particles
      if (phase === 1) {
        ctx.fillStyle = pal['Z']!;
        ctx.fillRect(ox + 3,  oy + 14, 1, 1); ctx.fillRect(ox + 12, oy + 15, 1, 1);
        ctx.fillRect(ox + 6,  oy + 16, 1, 1);
      }
    };
    wraith(6, 0); wraith(7, 1);
  ...
  private buildPortraitTextures(): void {
    ...
    portraits.forEach(([key, rows]) => {
      ...
      rows.forEach((row, gy) => {
        row.forEach((col, gx) => {
          if (col) { ctx.fillStyle = col; ctx.fillRect(gx, gy, 1, 1); }
        });
      });
      ct.refresh();
    });
  ...
  private drawElderPortrait(): string[][] {
    const _ = '';
    ...
    return [
      ...
      [G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G],
      [G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G,G],
    ]; _ as unknown as string; // satisfy TS
  }

  private drawMerchantPortrait(): string[][] {
    const G = '#3A5030';
  ...
  private drawAdventurerPortrait(): string[][] {
    const G = '#2A3840';
  ...
  private drawChildPortrait(): string[][] {
    const G = '#2A2050';
  ...
  private drawScientistPortrait(): string[][] {
    const G = '#202840';
  ...
  private simplePortrait(
    bg: string, H: string, h: string, S: string, c: string, K: string, g: string, O: string, o: string,
  ): string[][] {
    const _ = '';
    _ as unknown;
    const out: string[][] = Array.from({ length: 24 }, () => Array(24).fill(bg));
    // Hair
    for (let x = 6; x < 18; x++) { out[2][x] = H; out[3][x] = H; }
    for (let x = 5; x < 19; x++) { out[4][x] = H; out[5][x] = H; }
    out[3][5] = H; out[3][18] = H;
    // Face
    for (let y = 6; y < 12; y++) for (let x = 5; x < 19; x++) out[y][x] = S;
    // Eyes
    out[7][7] = K; out[7][8] = K; out[7][9] = g;
    out[7][14] = K; out[7][15] = K; out[7][16] = g;
    // Cheeks
    out[9][6] = c; out[9][17] = c;
    // Mouth
    out[10][9] = c; out[10][10] = c; out[10][11] = c;
    // Neck
    for (let y = 12; y < 14; y++) for (let x = 9; x < 15; x++) out[y][x] = S;
    // Clothes
    for (let y = 14; y < 24; y++) for (let x = 4; x < 20; x++) out[y][x] = O;
    for (let y = 14; y < 24; y++) { out[y][4] = o; out[y][19] = o; }
    // Hair sides
    out[6][5] = H; out[7][5] = H; out[8][5] = H;
    out[6][18] = H; out[7][18] = H; out[8][18] = H;
    out[5][6] = h; out[4][8] = h; out[4][14] = h;
    return out;
  }
  ...
  private buildEnvironmentSprites(): void {
    ...
    treeRows.forEach((row, gy) => {
      for (let gx = 0; gx < row.length; gx++) {
        ...
        if (col) { ctxT.fillStyle = col; ctxT.fillRect(gx, gy, 1, 1); }
      }
    });
    ctT.refresh();
    ...
    buildingRows.forEach((row, gy) => {
      for (let gx = 0; gx < row.length; gx++) {
        ...
        if (col) { ctxB.fillStyle = col; ctxB.fillRect(gx, gy, 1, 1); }
      }
    });
    ctB.refresh();
    ...
    gateRows.forEach((row, gy) => {
      for (let gx = 0; gx < row.length; gx++) {
        ...
        if (col) { ctxG.fillStyle = col; ctxG.fillRect(gx, gy, 1, 1); }
      }
    });
    ctG.refresh();
    ...
    const palS: Record<string, string | null> = { '.': null, Y: '#AACCFF', W: '#FFFFFF', y: '#88AADD' };
    crystalRows.forEach((row, gy) => {
      for (let gx = 0; gx < row.length; gx++) {
        const col = palS[row[gx]];
        if (col) { ctxS.fillStyle = col; ctxS.fillRect(gx, gy, 1, 1); }
      }
    });
    ctS.refresh();
  }
