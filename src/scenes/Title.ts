...
export class Title extends Phaser.Scene {
  ...
  private stars      : Array<{ gfx: Phaser.GameObjects.Rectangle; speed: number }> = [];
  private clouds     : Phaser.GameObjects.Graphics[] = [];
  private silhouettes: Array<{ obj: Phaser.GameObjects.Graphics; vx: number; y: number; phase: number }> = [];
  ...
  private buildNebula(): void {
    const nebData: Array<[number, number, number, number, number]> = [
  ...
  private buildCharacterSilhouettes(): void {
    const configs: Array<{ x: number; y: number; size: number; speed: number }> = [
  ...
  private drawPersonSilhouette(g: Phaser.GameObjects.Graphics, ox: number, oy: number, s: number): void {
    const col = 0x000008;
    g.fillStyle(col, 1);
  ...
  override update(_time: number, delta: number): void {
    ...
    this.silhouettes.forEach(s => {
      ...
      // Wrap around screen
      if (s.vx > 0 && s.obj.x > W + 30) s.obj.x = -30;
      if (s.vx < 0 && s.obj.x < -30)   s.obj.x = W + 30;
    });
