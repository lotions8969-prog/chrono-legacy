...
import { NpcScript, DialoguePage } from '../data/story';
...
export class DialogueSystem {
  ...
  private pageCounter : Phaser.GameObjects.Text      | null = null;
  private portraitImg : Phaser.GameObjects.Image | Phaser.GameObjects.Graphics | null = null;
  ...
  show(script: NpcScript, onClose?: () => void): void {
  ...
  private showPage(idx: number): void {
    ...
    if (this.portraitImg) {
      this.container.remove(this.portraitImg as Phaser.GameObjects.GameObject, true);
    ...
    // Update name
    if (this.nameText) this.nameText.setText(page.name);
    ...
    this.indicator?.setVisible(false);
    if (this.msgText) this.msgText.setText('');
  ...
  private scheduleNextChar(): void {
    ...
    this.typingTimer = this.scene.time.delayedCall(delay, () => {
      this.charIdx++;
      if (this.msgText) this.msgText.setText(this.fullText.slice(0, this.charIdx));
      this.scheduleNextChar();
    });
  }

  private charDelay(ch: string): number {
    if (ch === '.' || ch === '!' || ch === '?') return BASE_DELAY * 5;
    if (ch === ',' || ch === ';')               return BASE_DELAY * 2.5;
    if (ch === ' ')                             return BASE_DELAY * 0.5;
    if (ch === '\n')                            return BASE_DELAY * 3;
    return BASE_DELAY;
  }
  ...
  private finishTyping(): void {
    this.stopTyping();
    if (this.msgText) this.msgText.setText(this.fullText);
