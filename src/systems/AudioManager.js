...
export class AudioManager {
    constructor() {
        ...
        this.muted = false;
        // ── Lookahead scheduler ───────────────────────────────────────────────────
        this.LOOKAHEAD = 0.25; // seconds ahead to schedule
        this.INTERVAL_MS = 50; // scheduling interval
    }
    // ── Lifecycle ──────────────────────────────────────────────────────────────
    ...
    setMute(m) {
        this.muted = m;
        if (this.master)
            this.master.gain.value = m ? 0 : 0.22;
    }
    ...
    crossfade(track, durationMs = 600) {
        if (!this.ctx || !this.master)
            return;
    ...
    addChannel(notes, bpm, type, vol, detune, cutoff) {
        if (!this.ctx)
            return;
        this.channels.push({
    ...
    startScheduler() {
        const tick = () => {
            if (!this.ctx || this.channels.length === 0)
                return;
            const now = this.ctx.currentTime;
    ...
    scheduleNote(noteName, startTime, duration, ch) {
        if (!this.ctx || !this.master)
            return;
    ...
    sfx(name) {
        ...
        switch (name) {
            case 'select':
                this.tone('square', 440, 0.06, 0.05, 0.08);
                break;
            case 'cancel':
                this.tone('square', 280, 0.06, 0.02, 0.12);
                break;
            case 'attack':
                this.noise(0.15, 0.08);
                this.tone('sawtooth', 220, 0.15, 0.01, 0.15);
                break;
            case 'hit':
                this.noise(0.25, 0.06);
                break;
            case 'magic':
                this.sweep('sine', 440, 880, 0.3, 0.5);
                break;
            case 'heal':
                this.sweep('triangle', 523, 1046, 0.2, 0.4);
                break;
            case 'victory':
                this.fanfare();
                break;
            case 'level_up':
                this.sweep('square', 330, 660, 0.3, 0.5);
                this.tone('sine', 880, 0.3, 0.05, 0.4);
                break;
        }
    }
    tone(type, freq, vol, attack, release) {
        if (!this.ctx || !this.master)
            return;
        const ctx = this.ctx;
