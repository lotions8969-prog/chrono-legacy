...
function drawWindow(
  ...
  x    : number, y: number,
  w    : number, h: number,
  opts : { alpha?: number; borderColor?: number; innerColor?: number } = {},
): void {
...
export class BattleUI {
  ...
  private atbReadyDots     : Phaser.GameObjects.Graphics[] = [];
  ...
  private buildStatusPanel(playerUnits: BattleUnit[]): void {
    ...
    playerUnits.forEach((u, i) => {
      ...
      hpBg.fillStyle(0x1A0808); hpBg.fillRoundedRect(ox + 32, 14, 60, 5, 1);
      hpBg.lineStyle(1, 0x442222, 0.7); hpBg.strokeRoundedRect(ox + 32, 14, 60, 5, 1);
      c.add(hpBg);
      ...
      mpBg.fillStyle(0x080818); mpBg.fillRoundedRect(ox + 32, 23, 60, 5, 1);
      mpBg.lineStyle(1, 0x222244, 0.7); mpBg.strokeRoundedRect(ox + 32, 23, 60, 5, 1);
      c.add(mpBg);
      ...
      atbBg.fillStyle(0x080810); atbBg.fillRoundedRect(ox + 32, 32, 60, 5, 1);
      atbBg.lineStyle(1, 0x223344, 0.7); atbBg.strokeRoundedRect(ox + 32, 32, 60, 5, 1);
      c.add(atbBg);
  ...
  updateStatusPanel(playerUnits: BattleUnit[], levels?: number[]): void {
    if (!this.panel) {
    ...
    playerUnits.forEach((u, i) => {
      ...
      // Level
      if (levels) this.levelTexts[i]?.setText(`Lv${levels[i] ?? 1}`);
  ...
  private handleMenuInput(): void {
    ...
    if (JustDown(this.keys.z)) this.selectCurrent();
  }
  ...
  showTechMenu(
    ...
    const mpCostTexts: Phaser.GameObjects.Text[] = [];
  ...
  // ── Level-up effect ──────────────────────────────────────────────────────────
  showLevelUp(unitName: string, level: number): void {
  ...
  showVictoryExp(expGain: number, gold: number): void {
